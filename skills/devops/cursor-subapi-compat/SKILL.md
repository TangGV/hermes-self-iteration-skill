...
- **普通代码编辑/加注释任务读完文件后只说“我会…”不继续执行：** 用户语义如“给当前代码加注释/modify/fix”必须识别为 actionable。桥接已加入 `注释/comment/annotate` 触发词，并加 `_read_without_write_after_actionable_request()`：只要 actionable 请求已经 `ReadFile` 但尚无 `ApplyPatch/Write/Edit/EditNotebook/Delete`，就强制 `tool_choice=required`，避免模型用普通文字 stop。验证脚本 `scripts/test_comment_read_requires_edit_tool.py`。
- **Cursor Plan 读完 `.plan.md` 后 Reconnecting / 只说“我会修改”但不写回：** 这不是空流时，日志通常是 `has_tool_calls=False saw_text=True finish_seen=stop status=ok`，且前一轮刚 `tool_names=ReadFile` 读了 `*_hash.plan.md`。根因是模型读完计划后用普通文本 stop，没有继续 ApplyPatch/Write。桥接已加 `_plan_read_without_write()`：有 plan_lock 且已 ReadFile `.plan.md`、尚未 ApplyPatch/Write/Shell 写回时，强制 `tool_choice=required`，同时保留去除 CreatePlan，迫使下一轮编辑现有计划文件。
...
- **本机「CLI 分析流量」/ Context 0% vs 大 raw_len**：用户问「**我们不是做了流量分析吗**」→ **先 TrafficLens**（远程 **`win-start-cursor-vps1-remote.ps1`** / **`cursor-tl-launcher`** → VPS1 **`remote-cursor-mitm.jsonl`**；或 **`/tl/cursor/v1`** → **`cursor-debug.jsonl`**），再 **`cursor.requestTraces.log`**（`getBlob.slow` / `writeInitialRequest`），最后 **8327 `raw_len` + `usage_seen`**。Windows **没有** Cursor 内置明文抓包 CLI。Override **subapi/api `/cursor/v1` 直连时常不进 18888 JSONL**，不能因 TL 无行否认流量。若用户要求“本地用 CLI Composer 2.5 测试”，使用 `C:\Users\t\AppData\Local\cursor-agent\cursor-agent.cmd`（不是 GUI `cursor.exe agent` 包装器），用 `--print --trust --model composer-2.5` 跑 smoke/文件写入/代码修复/pytest/大文件 sentinel，并用 `--output-format stream-json` 确认 `model: Composer 2.5`；CLI 成功不等于 `/cursor/v1` 健康，必须并行查 SubAPI bridge `active-audit/flow-audit/resp-audit/slow-audit`。详见 **`references/trafficlens-triage-flow-context-and-empty-stream.md`**、**`references/cursor-request-traces-local-traffic-analysis.md`**、**`references/cursor-agent-cli-composer25-testing.md`**。
